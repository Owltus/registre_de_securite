...
interface DeleteSignatureSheetDialogProps {
  sheet: SignatureSheet | null
  onClose: () => void
  onConfirm: () => void
}

export function DeleteSignatureSheetDialog({ sheet, onClose, onConfirm }: DeleteSignatureSheetDialogProps) {
  return (
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[92vw] max-w-sm border bg-background shadow-lg rounded-lg flex flex-col overflow-hidden focus:outline-none">
          <div className="flex items-center justify-between border-b px-6 py-4">
            <Dialog.Title className="text-lg font-semibold">
              Supprimer la feuille de signature
            </Dialog.Title>
          ...
          <Dialog.Description className="sr-only">
            Confirmer la suppression de la feuille de signature
          </Dialog.Description>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
